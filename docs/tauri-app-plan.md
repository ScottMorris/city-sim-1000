...
- [ ] **T4-5 · Depot layout + steamcmd upload CI.** A `steam-depot` job producing the raw unpackaged tree (a fifth bundle shape the org has never made — Steam wants files, not installers) per T4-1's chosen approach, plus a steamcmd/SteamPipe upload to a beta branch on tag; secrets for the builder account. `deps:` T4-1, T2-2.
...
- **Desktop-only, gracefully.** Real implementation cfg-gated to Linux/Windows/macOS; Android/iOS get a stub that always reports unavailable, so shared codebases can depend unconditionally (same pattern as the workspace's platform-gated plugins).
- **Feature-gated linking.** The SDK must not be linked into non-Steam builds — consumers enable a cargo feature from their `steam` distribution channel only. Verify the `steamworks-sys` redistributable/licence story (SDK binaries in the workspace vs consumer-provided) before v0.1.

### Command surface (v0.1 → v0.3)
- **v0.1 — identity + achievements + stats:** `isAvailable()`, `getPlayer()` (steamId, personaName, appLanguage), `unlockAchievement(id)`, `clearAchievement(id)` (dev), `getAchievement(id)`, `indicateProgress(id, cur, max)`, `setStat`/`getStat`/`storeStats`, events: `stats-received`, `achievement-stored`.
- **v0.2 — presence + overlay + cloud:** `setRichPresence(kv)`, `clearRichPresence()`, `activateOverlay(page)`, `activateOverlayToWebPage(url)`, event `overlay-toggled`; Steam Cloud file API (`cloudWrite`/`cloudRead`/`cloudList`/`cloudQuota`) for apps not using Auto-Cloud.
- **v0.3 — nice-to-haves:** DLC queries, `isSteamDeck`, Steam Input glyph helpers, screenshots hook. Explicit non-goals for now: matchmaking/lobbies, networking, workshop, in-app purchases — design shouldn't preclude them, but they're separate issues.

### guest-js
Full typed TS API per the workspace template — `guest-js/index.ts` with typed wrappers over `invoke('plugin:steamworks|…')` and typed event helpers, rollup build to `dist-js` + `api-iife.js`, `package.json` exports, published to npm via covector like the existing plugins. The JS surface is the product here: consumers should never hand-write an `invoke` string.

### Permissions
Granular permission sets per command group — `allow-achievements`, `allow-stats`, `allow-presence`, `allow-overlay`, `allow-cloud`, `allow-player-info` — with `default.toml` granting only identity + achievements + stats; cloud and overlay opt-in. Autogenerated permission docs per the workspace build.

### Testing/CI
Steam can't run in CI: a `mock` cargo feature backs the same command surface with an in-memory fake (unlocks recorded, stats stored) so plugin unit tests and consumer e2e tests run Steam-less; manual verification against appid 480 (Spacewar) documented in the plugin README.

### First consumer
city-sim-1000 Phase T4 (`docs/tauri-app-plan.md`): achievements mirroring an engine-side milestone system, Auto-Cloud on its native save directory, rich presence ("Mayor of <city>, pop. 12,400"). Cross-link the city-sim epic when both exist.
