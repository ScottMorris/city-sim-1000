...
**Status:** engine model landed (`#230`). `recompute_utility_network` labels every reached tile with a connected-component id per pass, and tracks per-component produced/consumed alongside the existing city-wide totals.
...
* `UtilityComponent::utilization()` — `min(1.0, used / produced)`, the attribution convention for multiple sources sharing one segment: every source on a segment is assumed to run at the same utilization fraction as the segment itself. Chosen over merit-order dispatch (no per-plant cost/priority model exists) and distance-weighted attribution (would need a second, more expensive pass).
...
* Per-plant utilization surfaced in the building inspector UI, or any HUD/panel rendering of per-segment stats.
