...
//!
//! Now `Tile::terrain` holds the ground and three sibling fields — a
//! [`StratumSet`] per [`Layer`], one each for [`Underground`], [`Surface`] and
...
//!
//! Three strata, three fields, one type — see the `strata` module note below
//! for why they are a [`StratumSet`] per [`Layer`] and not three bare `u16`s,
//! and for why the layer is a marker type rather than a const parameter.
//!
...
///
/// So the field type is [`StratumSet<L>`](StratumSet): a newtype over
/// `OccupantSet` whose inner field is private, parameterised by the layer it
/// belongs to, in a module that is itself private. Outside this module there is
/// no `StratumSet::<Surface>(1 << 9, PhantomData)`, no `.0`, no `From<u16>` and
/// no transparent `Deserialize`. What exists instead:
///
/// | to obtain one | what stops a foreign bit |
/// | --- | --- |
/// | [`StratumSet::EMPTY`] / `Default` | it is empty |
...
/// lines long.
///
/// # One type, not three lookalikes
///
/// [`Underground`], [`Surface`] and [`Overhead`] used to be three structs
/// generated by a `macro_rules!` expanded three times. A macro is textual: the
/// compiler saw three unrelated types that merely resembled one another, and
/// nothing checked that they stayed coherent, because to the compiler they were
/// never related. `bits`, `is_empty`, `Default`, `Serialize` and — most
/// wastefully — the `Deserialize` impl below and the long argument attached to
/// it all existed in triplicate. They are written once here, over a [`Layer`].
///
/// **Why marker types and `PhantomData`, and not `StratumSet<const S: u8>`:**
///
/// - [`Stratum`] stays an honest enum. Const generics cannot take an enum on
///   stable — that is `adt_const_params`, still unstable — so the const-generic
///   form would have to launder the stratum through a `u8` and hand every
///   reader a number to decode back into the concept.
/// - `StratumSet<L>` can only be instantiated with a real [`Layer`]:
///   `StratumSet<u8>` does not compile. `StratumSet<7>` is a perfectly nameable
///   type that would need a *separate* const guard to rule out — a second
///   invariant, defending the defence.
/// - The trait doubles as the declared contract — "a layer is a thing that
///   names a stratum" — written in one place a reader can find. The macro had
///   that nowhere: its contract was the shape of an expansion.
///
/// [`Layer`] is left open rather than sealed, and that is not a hole — but be
/// precise about *why*, because the obvious reason is wrong. `Deserialize` does
/// check against `L::STRATUM`. [`place`] does **not**: it never reads
/// `L::STRATUM` at all. Its safety comes from being the single writer, taking
/// three concretely-typed parameters, and choosing the slot from
/// `occupant_def(occupant).stratum` — the occupant's own declaration — so it
/// cannot be *pointed* at the wrong set. A foreign `impl Layer` buys nothing
/// against that: it still has to name one of the three real [`Stratum`] values,
/// and it yields only a fourth spelling of a stratum [`Tile`] never stores.
///
/// **What would open a hole is a *generic* writer inside this module.** A
/// plausible-looking helper —
/// `fn place_in<L: Layer>(set: &mut StratumSet<L>, o: Occupant, on: bool)`
/// setting `set.0` directly — compiles with no diagnostic and would happily put
/// a `PowerLine` bit in a `StratumSet<Surface>`, reproducing the two-spellings
/// bug this model exists to end. Writers here must stay non-generic, and
/// `each_stratum_set_only_ever_holds_bits_in_its_own_mask` will not catch one
/// that isn't, because it drives [`Tile::set_occupant`] rather than an
/// arbitrary in-module write.
///
...
mod strata {
    ...
    /// One physical layer of the map, as a type — the parameter of
    /// [`StratumSet`].
    ///
    /// The contract is the whole trait: a layer is a thing that names a
    /// [`Stratum`]. `L::STRATUM` is what `place` routes by and what
    /// `Deserialize` validates against, so the marker is never a decoration —
    /// it is the fact the set is checked against, carried at the type level
    /// where it costs no bytes.
    pub trait Layer {
        /// The stratum whose occupants a `StratumSet<Self>` may hold.
        const STRATUM: Stratum;
    }

    /// The layer under the tile: [`Occupant::Pipe`], and the reserved `Subway`
    /// and `Fibre` tags. Depth is free, so this is the one stratum whose
    /// occupants coexist by default.
    ///
    /// A marker. It carries no data and is never stored — [`Tile`] stores
    /// `StratumSet<Underground>`.
    ///
    /// [`Tile`]: crate::state::Tile
    #[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
    pub struct Underground;

    impl Layer for Underground {
        const STRATUM: Stratum = Stratum::Underground;
    }

    /// The layer on the ground: road, rail, the three zone tags and
    /// [`Occupant::Structure`]. The scarce stratum — conflict is the default,
    /// and the level crossing is the single exception.
    ///
    ...
    impl Layer for Surface {
        const STRATUM: Stratum = Stratum::Surface;
    }
    ...
    impl Layer for Overhead {
        const STRATUM: Stratum = Stratum::Overhead;
    }

    /// The occupants of one layer of one tile.
    ///
    /// A newtype over [`OccupantSet`] with a private inner field, in a private
    /// module: see the module note for why, for the four ways a value can come
    /// into existence, and for why the layer is a marker type rather than a
    /// const parameter.
    ///
    /// `PhantomData<L>` is the whole cost of the parameter: the struct is one
    /// `u16` at runtime and one `u16` on the wire, exactly as the three
    /// macro-generated newtypes were.
    pub struct StratumSet<L: Layer>(OccupantSet, PhantomData<L>);

    impl<L: Layer> StratumSet<L> {
        /// The stratum whose occupants this set may hold. Restated from
        /// [`Layer::STRATUM`] so a reader holding a `StratumSet` can ask it
        /// without naming the marker.
        pub const STRATUM: Stratum = L::STRATUM;

        /// Nothing here.
        pub const EMPTY: Self = Self(0, PhantomData);

        /// The occupant bits, at their **absolute** [`Occupant`] positions.
        /// That is what keeps the union free:
        /// `underground.bits() | surface.bits() | overhead.bits()` is the whole
        /// tile, with nothing shifted into place.
        #[inline]
        pub const fn bits(self) -> OccupantSet {
            self.0
        }

        /// Whether the stratum is empty. There is deliberately no `contains`
        /// beside it: "is this occupant present?" is [`Tile::has_occupant`]'s
        /// question and it does not need a stratum, because the occupant
        /// already carries one.
        ///
        /// [`Tile::has_occupant`]: crate::state::Tile::has_occupant
        #[inline]
    ...
    // The ordinary value traits, hand-written rather than derived. A derive
    // would bound each impl on `L`, and `StratumSet<L>` is `Copy` and `Hash`
    // and everything else for reasons that have nothing to do with the marker:
    // the only field with a value is the `u16`. The marker is type-level, so
    // it must not appear in a bound.

    impl<L: Layer> Clone for StratumSet<L> {
        fn clone(&self) -> Self {
            *self
        }
    }

    impl<L: Layer> Copy for StratumSet<L> {}

    impl<L: Layer> Default for StratumSet<L> {
        fn default() -> Self {
            Self::EMPTY
        }
    }

    impl<L: Layer> PartialEq for StratumSet<L> {
        fn eq(&self, other: &Self) -> bool {
            self.0 == other.0
        }
    }

    impl<L: Layer> Eq for StratumSet<L> {}

    impl<L: Layer> Hash for StratumSet<L> {
        fn hash<H: Hasher>(&self, state: &mut H) {
            self.0.hash(state);
        }
    }

    impl<L: Layer> fmt::Debug for StratumSet<L> {
        /// Names the *set*, not the marker: `StratumSet<Surface>(0b…)`, because
        /// under this module's naming `Surface` alone is the place, not the
        /// occupants in it. Binary because a bitset is what it is.
        ///
        /// This is a deliberate change of rendering from the three
        /// macro-generated newtypes, which printed `Surface(0)`. Nothing
        /// asserts on it and it reaches no wire, but it is a visible difference
        /// in logs, so it is recorded rather than slipped in.
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            write!(f, "StratumSet<{:?}>({:#013b})", L::STRATUM, self.0)
        }
    }

    /// The bare [`OccupantSet`], and nothing else. The marker is type-level and
    /// contributes no bytes, so this writes exactly what `#[serde(transparent)]`
    /// wrote for the three macro-generated newtypes.
    ///
    /// Hand-written rather than derived for the same reason as the impls above,
    /// and one more: a derive over a `PhantomData<L>` field would demand
    /// `L: Serialize`, a bound on a type that is never serialised. Saying it by
    /// hand is shorter than saying `#[serde(bound = "")]` and explaining it.
    impl<L: Layer> serde::Serialize for StratumSet<L> {
    ...
    /// **Rejects the payload** when a bit does not belong to this stratum. Not
    /// masked-and-logged, and the choice is not close.
    ///
    /// The invariant is unrepresentable in the running program, so a violation
    /// cannot have been *written* by any build of this engine — not by a tool,
    /// not by [`super::strata::place`], not by the v4 decode, which goes
    /// through the same path. The only way to see one is a payload that is
    /// corrupt or forged. And postcard is non-self-describing: fields are
    /// positional, with no names, no lengths and nothing to resynchronise
    /// against, so a stratum holding a foreign bit is far likelier to mean *the
    /// reader and the writer disagree about where the fields are* than to mean
    /// one tile is slightly wrong. Every byte after it is then suspect too.
    ///
    /// Masking would launder that into a city which loads and is quietly wrong
    /// — the failure mode this model exists to end. Refusing costs a legitimate
    /// save nothing, because no legitimate save can contain one.
    ///
    /// This is a *different* question from [`super::validate_set`], which stays
    /// advisory: that one asks whether two occupants should share a tile, and
    /// saves in the wild genuinely do contain tiles it dislikes. This one asks
    /// whether the bytes are the struct they claim to be.
    ///
    /// **There is one of these now, not three.** It is the same argument for
    /// every stratum, and under the macro it was three copies of that argument
    /// with nothing keeping them in step.
    impl<'de, L: Layer> serde::Deserialize<'de> for StratumSet<L> {
        fn deserialize<D>(d: D) -> Result<Self, D::Error>
        where
            D: serde::Deserializer<'de>,
        {
            let bits = <OccupantSet as serde::Deserialize>::deserialize(d)?;
            let foreign = foreign_bits(bits, L::STRATUM);
            if foreign != 0 {
    ...
    #[inline]
    pub fn place(
        underground: &mut StratumSet<Underground>,
        surface: &mut StratumSet<Surface>,
        overhead: &mut StratumSet<Overhead>,
        occupant: Occupant,
        on: bool,
    ) {
...
pub use strata::{Layer, Overhead, StratumSet, Surface, Underground};
...
impl Tile {
    ...
    pub fn clear_stratum(&mut self, stratum: Stratum) {
        match stratum {
            Stratum::Underground => self.underground = StratumSet::EMPTY,
            Stratum::Surface => self.surface = StratumSet::EMPTY,
            Stratum::Overhead => self.overhead = StratumSet::EMPTY,
        }
    }
...
mod tests {
    ...
    ///
    /// `StratumSet::<Surface>(1 << 9, PhantomData)` does not compile here
    /// either: the inner field is private and `occupants::strata` is not
    /// exported, so bytes are the only way to attempt it. That is why the check
    /// lives in `Deserialize` and why it rejects rather than masks — see the
    /// impl for the argument.
    ///
    /// The single generic impl is checked against all three layers here, which
    /// is the point of it being single: under the macro these were three
    /// separate impls that only a test like this one held together.
    #[test]
    fn a_stratum_set_refuses_a_bit_that_is_not_its_own() {
        ...
        assert!(
            postcard::from_bytes::<StratumSet<Surface>>(&line).is_err(),
            "Surface must refuse an overhead bit"
        );
        assert!(
            postcard::from_bytes::<StratumSet<Underground>>(&line).is_err(),
            "Underground must refuse an overhead bit"
        );
        assert_eq!(
            postcard::from_bytes::<StratumSet<Overhead>>(&line)
                .unwrap()
                .bits(),
            B_POWER_LINE,
        ...
        for name in ["Underground", "Surface", "Overhead"] {
            let refused = match name {
                "Underground" => postcard::from_bytes::<StratumSet<Underground>>(&spare).is_err(),
                "Surface" => postcard::from_bytes::<StratumSet<Surface>>(&spare).is_err(),
                _ => postcard::from_bytes::<StratumSet<Overhead>>(&spare).is_err(),
            };
