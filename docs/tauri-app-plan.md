...
- [ ] **T0-5 · Fix #178 — full-fidelity tile mirror over the Channel.** *(wire format done; DoD verification still open, see below)* The tick event must carry the same SoA tile buffer the WASM path sends (`underground`, `surface`, `overhead`, `status`, `happiness`, `elevation`, `building_id`, `wilderness` — current strata shape, not the pre-#177 `kind`/`flags` spelling this line used to name), not the kind-byte projection that loses road/rail/hydro coexistence. Done in two steps: #178 (coexistence bytes, PR #187) and #204 (the remaining `building_id`/`happiness`/`elevation`/`wilderness` gap, PR #212) — both now share `city_sim_core::wire::encode_tile_buffer`/`protocol/tileBuffer.ts`'s `decodeTileBuffer` with the WASM path, not a second bespoke encoder.
  **DoD:** #178's three repro cases render identically under `?bridge=tauri` and the WASM path — not yet verified on-device, blocked on T0-1 (no Tauri host app exists in this tree yet). `tileRenderUtils` parity test extended to the Tauri event shape — not done; tracked as #213, since there's no cross-engine parity coverage for the Tauri transport at all yet, not just this one test.
