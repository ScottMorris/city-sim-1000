...
Production simulation runs in Rust. `simulation.ts` (TS) is retained as a test-only parity oracle — do not import it from production code. `adjacency.ts` provides road/rail/power-reachability queries used by the renderer and UI.
