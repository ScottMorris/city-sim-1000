...
- Bylaws button in the status ribbon (next to Budget/Settings) to set a city-wide lighting standard that scales civic/zone power demand and upkeep — engine-simulated and reflected in real time on the budget ledger (district overrides are on the way).
- Wilderness score (0–100) in the status ribbon with a trend arrow and breakdown tooltip: contiguous forests/parks, waterfront greenery, and open land versus industry, transport, and power infrastructure. Every feature a tile carries is scored, so a hydro line strung over a road costs you the road *and* the line — each attributed to its own line in the breakdown — and running lines along your roads no longer flatters the score. High scores attract residents and pay a tourism dividend; Bylaws offers Nature Reserve and Green Industry programmes, and the minimap gains a wilderness heatmap overlay.
...
- Quick bulldoze: right-click to bulldoze the tile under your cursor, or hold and drag to clear a path. Middle-click or Alt-drag still pans. The Inspect tool does not bulldoze on right-click. The bulldozer clears what is built on a tile and leaves the terrain alone — a bulldozed lake is still a lake; use Raise (`E`) to drain one.
