...
- **Adjacency draft**:
  ...
  - Water: 4-way flood fill through pipes and water facilities (Pumps/Towers). Powered sources seed the network; a Pump additionally needs its footprint orthogonally adjacent to a water tile (`#200`) — a dry pump is `InactiveNoSource` and seeds nothing, while a Tower is terrain-independent by design. Roads and rail also conduct water (early game); underground pipes connect sources to the network and bridge gaps. Buildings without water connection become Inactive.
...
- **Maintenance vs revenue**: How upkeep scales relative to income; target cadence of build → wait → build. Transport upkeep is charged once per *feature* a tile carries, not once per tile: a road with a hydro line over it pays 0.1 + 0.08, and a level crossing under a line pays 0.1 + 0.2 + 0.08. Water pipes are billed from the underground layer.
...
- **Baseline toggles**: City-wide lighting bylaws are live and engine-simulated (`LightingPolicy` in `crates/city-sim-protocol/src/commands.rs`, part of `Policies`). Mixed corridors keep a neutral baseline (both multipliers exactly `1.0`); energy-efficient lighting trims civic/zone power use (18%) and upkeep (10%); heritage/carbon-arc lamps raise power draw (18%) and upkeep (5%). Each option also carries a "mood target" number shown in the modal as a preview delta — decorative only; no simulation mechanism currently consumes it (see the Simulation impacts note below). The modal reads the engine's real, already-applied power/upkeep figures off the wire and shows what each option would change them to.
...
- **Simulation impacts**: Lighting bylaws scale civic + zone power draw (`Simulation::compute_utility_use`) and civic + zone maintenance (`economy::compute_daily_budget`) — never transport/pipes/power-line upkeep, power plant maintenance, or power *production*. The "mood target" number is display-only: it is not wired into `wilderness::apply_happiness_drift` (the actual mechanism that drifts zone-tile happiness, driven by the wilderness score, not the lighting bylaw) or any other simulation figure — wiring it in would be new simulation behaviour beyond what this feature has specified so far. Districts provide a hook for future policies (noise, pollution, density caps) by storing per-district modifiers alongside zoning data.
