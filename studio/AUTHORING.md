...
- **Wires, cables and anything that hangs** (`scenes/power.py` is the worked example). Three rules, each learned by getting it wrong:
  - *Sag is directional, and that isn't a cheat.* A span running away from the viewer is foreshortened and its droop collapses to nothing, so draw it straight; a span running across the view shows its full catenary. Applying one treatment to both is what forces the two-projection split that makes corners impossible.
  - *The curve's vertex belongs at the span's midpoint, not at the support.* A cable is **steepest where it is tied** — tension hauls it up to the arm — and **flattest at its lowest point**. Since props stand at tile centres, that midpoint is the tile edge, so the correct curve has zero slope at the seam and neighbouring tiles meet tangentially: a run reads as one continuous wave, peak at each pole, trough at each seam. Put the vertex at the pole instead (the naive `y = anchor - sag·x²`) and the span still descends — it looks like sag at a glance — but it is flat where it should be steep, giving a smooth hump over every pole and a sharp V-kink at every tile boundary. Use `y = anchor - sag·(1 - (1-t)²)` with `t = |x| / HALF`. *Descending is not enough; where the curve is flat and where it is steep is what makes it read as hanging.*
  - *Nothing hangs higher than what it's tied to.* Anchor a span above its attachment point and it bulges over the support, putting the apex at a local maximum — a run then reads as a row of arches rather than a hanging line. Peak at the tie; sag to the midpoint of the span. Since props stand at tile centres, that midpoint is the tile edge, which also makes tiles join seamlessly.
  - *Everything meets at the attachment.* Legs that stop short of the prop leave nothing for a corner or junction to be built from, and read as unrelated runs crossing.
- **A prop standing on infrastructure must clear it — and how far it can move is set by the prop's own height, not by taste.** A hydro pole on a road tile cannot stand in the traffic lane, but the tile art is full-width asphalt, so there is nowhere obvious to put it. Measure before choosing: the road sprites put their verge band at `|1.75|`–`|2.0|` world, and the composited pole occupies `y -1.07` (foot) to `+1.60` (cap) — 2.67 tall in a 4.0 tile. Sideways it moves freely (`1.30` lands its far edge hard against the kerb, still inside the tile). Vertically it can only ever go **south**, and only to `-0.80`: there is no offset that puts the foot on the *north* verge without throwing the cap clean off the tile. So an east-west carriageway always carries its line down the south side. The arm then sits near the road's centre line, and that is right rather than a compromise — a real pole stands at the kerb and cantilevers its crossarm out over the carriageway. **What the eye reads as "in the road" is the foot.**
- **A tile's sprite set is the product of its own connectivity and what it was laid over, and that product is bigger than it looks.** Hydro has 16 connectivities (15 road-style variants plus a bare pole) against 8 substrates, which is 128 cases — 103 of which needed the pole moved. Enumerate the product in a test and classify every outcome (`app/src/rendering/hydroCoverage.test.ts` prints the matrix); do not sample it by screenshot, which missed whole families twice. Parameterise the scene by *situation* rather than authoring cases by hand: `power.py` takes a carriageway class (`along-ns`, `along-ew`, `junction`) crossed with the base variant, decides the kerb side from the same resultant-pull sum that decides the guy, and the game addresses sprites by (variant, class) alone — it never has to know which side was chosen.
- **A leg running along the carriageway meets a displaced neighbour; a leg running off it meets an undisplaced one.** So the first terminates at the *displaced* edge point and the corridor reads straight, and the second terminates at the edge *midpoint*. That keeps every seam joined. A line entering or leaving a corridor jogs sideways once, which is what real distribution does anyway.
- **Compositing offsets belong to the scene, not the stylizer.** Where a billboard prop stands and where the wires meet its arm are one decision; stating it in both `power.py` and `stylize.mjs` meant two files had to agree, and eventually they didn't. The scene now exposes `prop_offsets()`, `render-passes.py` writes it to `props.json` beside the passes, and the stylizer just reads it.
- **Screen-space is a legitimate escape hatch when plan-space produces nonsense.** Guy wires are the case: a guy always descends to the ground, and its compass bearing only chooses which side of the pole it lands on. Placing its anchor at the plan position sends a south-braced pole's guy *up* the image. So guys are drawn in screen space — arm down to the prop's foot, offset laterally — and the departure is commented at the point of use. Prefer the rules, but when one of them would produce a worse sprite, break it deliberately and write down why.
