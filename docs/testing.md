...
The same rule held for the pinned known drifts in the now-retired cross-engine parity harness (see "Harness 2" above).

The clearest illustration of why is one that no longer exists as a fixture: `crates/city-sim-core/tests/fixtures/city_v4.csim`/`city_v4.expected` were a genuine pre-strata save, generated on the commit `fix(sim): read every stratum, so no feature goes uncounted` by a test that was deleted immediately afterwards — a save this tree could never produce again, proving the v4 → v5 migration against a file with no regenerate button at all. Both files and the test that read them (`snapshot::v4_fixture`) were deleted once the live wire itself moved onto occupant bits, because the fixture's entire premise — proving the wire *didn't* change across the #177 step-3 migration — was exactly what that flip un-proved; see `crates/city-sim-core/tests/fixtures/README.md`. Old `.citysim` saves still import byte-for-byte, now through the separately-frozen `legacy_tile_buffer.rs`, exercised by `import.rs`'s own round-trip tests instead. A migration you can only demonstrate against your own output was not a migration, and deleting the fixture rather than widening what it pinned is what that rule looks like followed all the way through.
...
| **Save/load has no round-trip harness of its own** | Nothing writes a save on this tree, reloads it, and asserts the city is identical. `import.rs`'s round-trip tests cover legacy JSON → engine import; nothing round-trips a live CSAV container — engine snapshot plus `ClientState` settings. | Low. A golden-city variant that snapshots, reloads and re-dumps would be a day's work and would reuse the existing dump code. |
