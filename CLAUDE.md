...
All simulation access goes through the `SimBridge` interface: `step`, `send`, `onMessage`, `getState`, `loadState`, `setSpeed`, `dispose`. `WasmSimBridge` routes through a Web Worker running a WASM `SimHost` (default browser path — requires `bun run build:wasm`). `TauriSimBridge` routes through `tauri-plugin-city-sim` — auto-detected via `window.__TAURI_INTERNALS__` or forced with `?bridge=tauri`. The active bridge is chosen at startup in `main.ts`. `LocalSimBridge` has been removed (P5-4); `simulation.ts` is retained as a test-only parity oracle.
