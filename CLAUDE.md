...
Production simulation runs in Rust; it is the sole engine and the sole spec. The browser/desktop clients send `SimCommand`s to the real Rust engine over the WASM/Tauri bridge, never through `tools.ts`'s `applyTool`. `simulation.ts`, the TS parity oracle that once mirrored it for tests, has been removed — its last version is preserved at commit `1f8140a` (`git show 1f8140a:app/src/game/simulation.ts`). `adjacency.ts` provides road/rail/power-reachability queries used by the renderer and UI.
