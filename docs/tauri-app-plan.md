...
- **Steam overlay over Tauri is known-broken upstream** ([tauri#6196](https://github.com/tauri-apps/tauri/issues/6196), closed as not planned): the overlay must hook the graphics device before creation, which webview initialisation timing prevents. Overlay commands will therefore be best-effort with a documented caveat — achievements, stats, presence, and cloud need no overlay.
