...
* `app/src/game/education.ts`'s `recomputeEducation` is deleted. `computeEducationReach` (the school-placement preview helper — no engine equivalent, since it previews a building that hasn't been placed yet) stays. `getEducationScore` and `createEmptyEducationStats` were later deleted too, as dead code once nothing called them — `debugStats.ts` reads `state.education?.score` directly, and `createInitialState`'s own defaults (`score`/coverages = `1.0`, not `0`, matching `EducationStats::default()` — a city with no schools has no load anywhere, which is full coverage, not zero) are the only place that shape gets constructed.
* **Wire-adoption follow-up:** the renderer's selected-school highlight used to run `computeEducationReach`'s BFS even for an already-placed school, ignoring the per-tile `services.served` it already had off the wire. `education.ts`'s `servedZoneTiles` reads that wire truth directly for the placed case; `computeEducationReach` is now reserved for the not-yet-placed ghost preview only, where it remains the one legitimate BFS (see its doc comment).
