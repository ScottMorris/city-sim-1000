...
| `ts-test` | none — carried cross-engine parity until the harness was retired | This job installs the Rust toolchain with the `wasm32-unknown-unknown` target, restores the Rust cache, installs `wasm-pack` and runs `bun run build:wasm` before `bun run test:ci`, the same shape `mobile-e2e` already used. That build step existed for the retired parity harness; no TS test needs it any more, but removing it from `ci.yml` is deferred to a later change. |
