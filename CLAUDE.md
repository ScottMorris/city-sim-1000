...
`GameState` is one large serializable object: a flat tile grid, utility stats, demand stats, budget stats, settings (minimap/input/accessibility/audio/hotkeys/cosmetics/narrative), bylaws, and building/service sub-states. `Tile` mirrors Rust's stratum model field-for-field — `terrain` (`Land`/`Water`) plus `underground`/`surface`/`overhead` occupant bitsets (`protocol/occupants.ts`'s `Occupant` enum: pipes, road, rail, zone tags, structure, power line, trees), not a single `kind` field — see `docs/tile-model.md`. Tiles reference buildings and power plants by id rather than embedding them. `TileKind` no longer keys building templates either: `buildings/templates.ts`'s `BuildingTemplate.kind` field is typed `BuildingKind`, a template-only enum with the same string values as `TileKind`'s buildable members, so save/MCP spelling didn't move. `TileKind` survives only in the frozen legacy wire path (`protocol/tileKind.ts`'s u8 mapping, `protocol/legacyTileBuffer.ts`, `persistence.ts`'s transcode) and as the `Tile` model's own enum declaration/test-only `setTile` helper in `gameState.ts` — not as a per-tile field or a live display derivation. `protocol/legacyProjection.ts`, the last TS port of the v4 `kind`+flags precedence ladder, was deleted once the renderer, minimap, and `mcpBridge.ts` converted to reading occupant bits (and `BuildingTemplate.kind`) natively. Settings have `createDefault*` factory functions in this file that `main.ts`'s `ensureSettingsShape` merges over loaded saves, so new settings fields must get a default factory here or old saves break.
...
TS mirror of `crates/city-sim-protocol` — `TileKind ↔ u8` mapping (template-key/legacy-save use only) and tile buffer SoA offsets. JSON-shaped wire types (`Policies`, `CommandResult`, `SimAlert`, `BuildingKind`, the `Wire*` stat structs) are ts-rs GENERATED from the Rust structs' own `#[derive(TS)]` into checked-in `protocol/generated/` (and `crates/tauri-plugin-city-sim/guest-js/generated/` for the Tauri path); a CI step fails if those directories drift from a fresh `cargo test -p city-sim-protocol -p tauri-plugin-city-sim` regen (`.github/workflows/ci.yml`). `SimCommand` and `FromSim` (`commands.ts`/`events.ts`) are TS-only envelope types composed from those generated pieces plus hand-written tool/client variants — Rust has no `SimCommand` or `FromSim` enum of its own. The live wire buffer (`tileBuffer.ts`) carries occupant bits directly, one byte per stratum, plus per-tile status/happiness/elevation/building-id/wilderness/education fields — 11 bytes/tile total (`BYTES_PER_TILE` in both `tileBuffer.ts` and `crates/city-sim-protocol/src/tile_buffer.rs`); the frozen v4 layout old `.citysim` saves use lives separately in `legacyTileBuffer.ts` (8 bytes/tile) and never changes. `tileKindParity.json`, `occupantParity.json`, and `tile_buffer_golden.json` are parity fixtures checked by `cargo test -p city-sim-protocol` and the corresponding tests in `app/src/game/protocol/`.
...
`templates.ts` defines static building specs (and `getToolCost` for UI display), `state.ts` defines runtime `BuildingInstance`. Placement and per-tick status updates run in the Rust engine; there is no TS-side building manager. Every numeric field on a `BuildingTemplate` (footprint, maintenance, power/water use, water output, population/jobs capacity, service radius/capacity) and every `Tool`'s build cost (`constants.ts`'s `BUILD_COST`) is read from `buildings/templateData.json`, generated from `city-sim-core`'s own `BuildingTemplate` table and `tool_cost` by `crates/city-sim-core/tests/template_data.rs` — Rust is the sole source for these numbers now; `templates.ts` keeps only display-only fields (name, colour, sprite key, ledger category).
...
Saves are binary CSAV containers: `"CSAV" | version | meta JSON | engine snapshot (CSIM postcard from `SimHost.get_snapshot`) | client JSON (`ClientState` — just `{ settings }` now the lighting bylaw moved into engine-owned `Policies`, see `clientState.ts`)`. Browser saves live in IndexedDB (`saveStore.ts`); downloads are `.citysim` files. Old-save import is one conversion, not a round trip: pre-CSAV legacy JSON saves (`isLegacyJsonSave` sniffs the magic byte) go through `transcodeLegacySave`, which reads the raw parsed JSON straight into the frozen v4 wire buffer (`legacyTileBuffer.ts`) and import scalars with no intermediate strata `GameState` — Rust's `tile_from_v4` (`city_sim_core::migrate`, driven by `import.rs`'s `from_tile_buffer`) is the sole place a v4-shaped tile is decoded into strata. `bridge.importLegacy` carries the result to `SimHost.import_legacy`. There is no legacy export — old saves are read-only from here on. When adding a client-owned state field, extend `ClientState`; engine-owned fields belong in the Rust `GameState` (bump the CSIM snapshot `VERSION`).
