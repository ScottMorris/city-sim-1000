...
- **Storage format: Opus; PCM only in flight.** Kokoro emits 24 kHz mono float32 PCM, which is ~48 KB/s even down-converted to 16-bit WAV — a 20 s spot is ~1 MB, so a modest spot library would blow the cache budget. Speech-tuned Opus at ~32 kbps mono is ~4 KB/s (~70 KB for the same spot): a ~15× capacity win for the same LRU budget at transparent quality for voice. The voice worker encodes immediately after synthesis and only the Opus blob is stored; raw PCM exists solely in flight between synthesiser and encoder. Codec choice (settled): **WASM libopus for both encode and decode** — the one path that behaves identically in every browser. The alternatives each fail somewhere: `MediaRecorder` can't encode faster than realtime, WebCodecs `AudioEncoder` is patchy on WebKit, and WebKit's `decodeAudioData` support for ogg/opus is historically unreliable. A single WASM codec sidesteps the whole compatibility matrix, keeps output byte-identical across platforms, and its few hundred KB of WASM load only when the voice pack is enabled (WebCodecs can be revisited later as a pure optimisation behind the same interface). Playback decodes once into an `AudioBuffer` and plays through `speechBus` as before. On encoder failure the entry falls back to WAV storage — voice never breaks over a codec.
...
- **R1 — segment-aware transport and captions.** Prev/next become segment operations: next skips the current segment (tracks and flavour spots yes, `emergency` spots no), prev steps back within a music block. The marquee shows the current caption line during spots (it already carries `role="status"`, so screen readers follow for free) and track metadata otherwise; the popover adds the station name/frequency, the segment's kind badge (📻 STATION ID / ⚡ EMERGENCY, per the demo), and the full script while a spot plays. Cover art falls back to station art for segments that have none (patterns, spots). **Play/pause becomes "tune out/tune in": the broadcast clock keeps running while paused** — resume fast-forwards the schedule to "now" rather than resuming mid-word, so the city keeps broadcasting whether or not the player listens. Compact mode has no marquee, so captions there live in the pinned popover, with news-kind spots already covered ambiently by the ticker cross-post.
- **R2 — a short transcript.** The pinned popover keeps the last few spots as a scrollable transcript (bounded, session-only), so a player who glanced away can catch the bulletin they half-heard. An `emergency` takeover also sets a widget-level state (the existing `data-state` attribute pattern) for a visible EBS treatment — the takeover must be legible with sound off.
- **R3 — voice affordances.** A small live-voice indicator while `speechBus` is active, and a caption-tap shortcut into the speech settings. The voice-pack install/benchmark flow lives in the settings modal, not the widget.
- **R4/R5 — the dial.** The station switcher in the popover evolves into the tuning dial (with the undertone static sweep between frequencies), and pirate/unlisted stations appear only as unlabelled stops on it. A Studio entry point (for stations the player owns) links into the settings-surface Studio rooms.
